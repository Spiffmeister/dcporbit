...
# Plot
pav = plot(xlabel="x",ylabel="y",legend=:topleft,framestyle=:classic,dpi=600,foreground_color_grid=:lightgray)

# Particle 1
gc1 = plot!(pav,gcsim₁.x[1,:],gcsim₁.x[2,:],color=cp[1],label="GC only z<0")
fo1 = plot!(pav,f.sp[1].x[1,:],f.sp[1].x[2,:],color=cp[1],linealpha=0.4,label="")

# Particle 4
gc2 = plot!(pav,gcsim₂.x[1,:],gcsim₂.x[2,:],color=cp[5],label="GC only z>0")
fc2 = plot!(pav,f.sp[4].x[1,:],f.sp[4].x[2,:],color=cp[5],linealpha=0.4,label="")


# Crossing particle 1
gcp, gcm = comp_avtraj(f.sp[2])
cross1 = plot!(pav,gcp[1,:],gcp[2,:],color=cp[3],label="GC Crossing")
plot!(pav,gcm[1,:],gcm[2,:],color=cp[3],label="")
crossav1 = plot!(pav,fe.sp[2].avetraj[1,2]*fe.sp[2].t,fe.sp[2].avetraj[2,2]*fe.sp[2].t,color=cp[3],linestyle=:dash,label="Average drift")
...
savefig(pav,"Figures//Presentations//gcav.pdf")
